...
def stamped_actor_gradients(policy, params, s, a, adv_r, adv_c, stamp):
    ...
    `stamp` already folds in b(i)/q(i) * W_i, where W_i = xi_i * w_i is the
    FULL off-policy weight of Eq. (2): the action-probability ratio w_i times
    the state-visitation density ratio xi_i = d^pi(s_i)/d^b(s_i) supplied by
    vosr_iclr.density_ratio. Passing only w_i here (the AAAI-era bug) leaves
    the state mismatch uncorrected and makes gr, gc biased estimates of the
    on-policy gradients, breaking Lemma 1.
    """
...
def policy_logp_batch(policy, params, s, a):
    ...
    from vosr_iclr.networks import gaussian_logp, EPS, LOG_STD_MIN, LOG_STD_MAX
